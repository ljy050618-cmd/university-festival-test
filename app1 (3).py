...
RESULT_MAP = {
    ...
    (1, 0, 0): {
        ...
        "prescription": [
            ...
            "연애를 하고 싶다면 역시 플러팅이죠",
            "간식 처방: 달달한 간식 하나 들고 가서 플러팅 하고 오세요!."
        ],
    },
    (0, 1, 0): {
        "title": "실수로 대학을 왔어요",
    ...
    (1, 1, 1): {
        ...
        "prescription": [
            ...
            "간식 처방: 달달한 도넛과 함께 생활을 되돌아보는 여유를 가지는 건 어떨까요?."
        ],
    },
}
...
def get_result_key():
    love_high = 1 if get_section_score("love") >= 45 else 0
    grade_high = 1 if get_section_score("grade") >= 45 else 0
...
# =========================================================
    ...
    with nav_col1:
        ...
        if st.session_state.page_index > 1:
            prev_clicked = st.button("이전 페이지", use_container_width=True)
    
    with nav_col2:
        next_label = "결과 보기" if st.session_state.page_index == 3 else "다음 페이지"
        next_clicked = st.button(
            next_label,
            disabled=not all_answered(section_key),
